...
- **GitHub Action / cloud adapter (V1.1)**: implemented as a read-only acquisition/transport surface. It reads PR metadata, changed-file names, and check-run evidence through GitHub REST, never checks out or executes PR code, and publishes only a hash-bound advisory JSON bundle plus Step Summary/output fields.
...
- **CFI V1.1**: CI Failure Intelligence classifies exact hash-bound CI evidence as `NO_TERMINAL_FAILURE | EXPECTED_FAILURE_ONLY | UNEXPECTED_FAILURE_OBSERVED | INSUFFICIENT_EVIDENCE`. `diagnosis_eligible=true` means only that complete unexpected-failure evidence exists; it does not claim root cause, regression attribution, or repair correctness.
- **EIA V1.1**: External Intelligence Automation emits only `READY | NO_ACTION | BLOCKED` plus a deterministic idempotency key. `READY` is bounded to `CI_FAILURE_DIAGNOSIS` on current, complete evidence and carries `AUTOMATION_ADVISORY_ONLY`; it grants no worker, comment, approval, merge, or repository-write authority.
...
- **V1.1 expansion boundary**: Change Impact, CFI/EIA, direct Dev MCP projection, and the read-only GitHub Action/cloud adapter are additive V1.1 surfaces. Semantic-reviewer redesign, worker execution authority, automatic repair, comment publication, approval, merge, and release remain outside Repository Intelligence authority.
...
6. **`analyze_ci_failure_intelligence(snapshot)`**: Deterministic CI-failure triage over the existing hash-bound CI evidence contract. It distinguishes expected, unexpected, absent, and incomplete failure evidence under `CI_EVIDENCE_ONLY` without inferring root cause.
7. **`plan_external_intelligence_automation(snapshot|cfi_report)`**: Deterministic unattended-action advisory. It emits `READY` only for current-base, non-stale, complete unexpected CI failure evidence; stale or incomplete evidence fails closed to `BLOCKED`. Claim ceiling: `AUTOMATION_ADVISORY_ONLY`.
...
python -m reviewer.intelligence_cli --operation eia --input snapshot.json

# Read from stdin
cat snapshot.json | python -m reviewer.intelligence_cli --operation readiness --input -
```

## GitHub Action / cloud execution (V1.1)

This repository also exposes a composite `action.yml` for generic GitHub repositories. It deliberately does **not** run `actions/checkout`: the action reads only pull-request metadata, changed-file names, repository/default-branch identity, and check-run evidence through the GitHub REST API, then invokes the same canonical Repository Intelligence Core used by the local CLI.
...
The generated `reviewer.repository_intelligence_cloud.v1` bundle includes exact revision identity plus readiness, CFI, and EIA reports, is hash-bound by `content_sha256`, and has top-level `ADVISORY_EVIDENCE_ONLY`. The action also writes a GitHub Step Summary and outputs `readiness`, `cfi-status`, `eia-decision`, `claim-ceiling`, and the evidence path/hash. It never comments, approves, merges, dispatches a worker, or executes pull-request code. A consumer that wants automatic diagnosis must separately decide how to honor the EIA advisory and must supply its own execution authority.
