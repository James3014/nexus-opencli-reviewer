...
class PRSnapshot:
    ...
    def from_dict(cls,d:dict[str,Any],main:str):
        ...
        def declared(key):
            ...
            m=re.search(rf'(?:exact\s+)?{key}\s*[:=]\s*[` ]*([0-9a-f]{{40}}|[0-9a-f]{{64}})\b',body,re.I)
            return m.group(1) if m else None
        return cls(d['repository'],int(d['pr_number']),d.get('title',''),d.get('state','OPEN'),bool(d.get('draft',False)),d.get('mergeable'),d.get('base_branch','main'),d['base_sha'],d.get('head_branch',''),d['head_sha'],main,tuple(d.get('changed_files',[])),tuple(d.get('issue_numbers',[])),tuple(d.get('labels',[])),body,tuple(CheckObservation(**x) for x in d.get('checks',[])),d.get('observed_at',''),d.get('source_identity','fixture'),declared('base'),declared('head'),declared('main'),bool(d.get('expected_failure',False)),bool(d.get('do_not_merge',False)),bool(d.get('collection_complete',True)),tuple(d.get('collection_errors',[])),d.get('created_at',''),d.get('updated_at',''))
@dataclass
