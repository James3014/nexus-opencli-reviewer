...
class OpenCLITransport:
    ...
    def invoke(self,prompt):
        ...
        try:
            ...
            if not isinstance(env,dict) or not isinstance(env.get('response'),str): raise ValueError
        except (ValueError,json.JSONDecodeError): return TransportResult('OPENCLI_PROCESS_FAILURE',p.stdout,argv=args)
        return TransportResult('REVIEW_COMPLETED',env['response'],version=self.version(),envelope=env,argv=args)
    def version(self):
