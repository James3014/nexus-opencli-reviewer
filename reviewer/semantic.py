...
def parse_response(text):
    ...
    for f in value['findings']:
        if not isinstance(f,dict) or set(f)-allowed or f.get('severity') not in {'CRITICAL','HIGH','MEDIUM','LOW'}: raise SemanticParseError('REVIEW_PARSE_FAILED')
        if any(not isinstance(f.get(k,''),str) or len(f.get(k,''))>MAX_TEXT for k in ('category','evidence','reason','recommended_action')): raise SemanticParseError('REVIEW_PARSE_FAILED')
    return value
